...
class TrafficMetrics:
    def __init__(self, world, roi, roi_radius=50.0, draw_roi=True, csv_path="traffic_metrics.csv"):
        ...
        if not os.path.exists(self.csv_path):
            with open(self.csv_path, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["hour_start", "vehicle_flow", "avg_speed", "stopped_vehicles", "waiting_time"])
    ...
    def save_to_csv(self, hour_start):
        """Guarda una fila en el CSV."""
        print(f"Guardando métricas de tráfico para la hora que empieza en {hour_start} segundos.")
    ...
    def update(self):
        ...
        sim_time = world_snapshot.timestamp.elapsed_seconds

        # Guardar cada hora
        if sim_time - self.last_save_time >= 36.0:
            self.save_to_csv(self.last_save_time)
